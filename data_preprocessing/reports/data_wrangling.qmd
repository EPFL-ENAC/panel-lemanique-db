...
Based on the `IDNO` variable, the CSV file in a strict subset of the SPSS file, with all 5534 samples from the SPSS file found in the CSV. The subset is based on the `Particip_v2` variable, detailing the amount of participation to the study. Participants with a complete (defined as having seen more than 95% of the questions) or incomplete (defined as having seen between 6 to 94% AND having answered at least the first question) participation were selected.

#### HERUS

Contrary to what is written in the READ_ME, the `HERUS` is not a subset of the FORS data that is limited to participants from Vaud, Geneva or France, as it includes 384 samples coming from Valais (*n* = 76) and from Fribourg (*n* = 308). How exactly this subset was created is unclear. Part of the exclusion criteria must have been the `ParticipV2` variable, as the `HERUS` subset only includes samples with a complete participation.
...
-   `weight_tot` is an undocumented weighting variable, used to create some figures
-   `Groupe2` is a grouping variable with a finer granularity than `HS_ZONE_C`, created from `Groupe` in `figures_edition_main.R`
-   `Groupe3` is a grouping variable created from `Groupe` in `figures_edition_main.R`
...
```{r wave2-extravar-FORS}
#| results: asis
#| echo: false

cat(
  sprintf(
    "The FORS CSV data contains %i columns that are not included in any HERUS data source.",
...
Are there some variables that should stay constant throughout the waves? Groupe? GP_Age_source? CP_source? Localité_source?
